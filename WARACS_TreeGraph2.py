...
#     1. Check if xtg-file present
        if not _isfile(self.inFn):
            _exit("  ERROR: .nex file not found.")
#     2. Generating outpath
        outPath = CSO.rmext(self.inFn) + ".xtg"
#     3. Performing nex2xtg via Treegraph2 and reporting output/error
        cmdL = ["java -jar", self.pathToTG2, "-convert", self.inFn, "-xtg", outPath]
        CFO.extprog(cmdL)
...
#     1.a. Check if xtg-file present
        if not _isfile(self.inFn):
            _exit("  ERROR: .xtg file not found.")
...
# 1.2. Loading color dictionary and running rudimentary checks
    ...
    for line in colorTmp:
        tmp = line.split(",")
        if len(tmp[0]) != 1:
            _exit("  ERROR: Problem with area codes in color dictionary.")
        if tmp[1][0] != "#" or len(tmp[1]) != 7:
            _exit("  ERROR: Problem with color codes in color dictionary.")
        colorDict[tmp[0]] = tmp[1]

#   2.1. Conversion from .nex to .xtg format
    if verbose.upper() in ["T", "TRUE"]:
        print "  Step 1: Conversion .nex -> .xtg"
    ConversionNEX2XTG(treeFn, pathToTG2).go()
    xtgFn = CSO.rmext(treeFn) + ".xtg"
    out_step1 = CFO.loadR(xtgFn)
    CFO.deleteFile(xtgFn)
